...
const Cart = () => {
    ...
    const { isOpen, items, isAddress, isCart } = useSelector((state: RootReducer) => state.cart)
        ...
        <CartContainer className={isOpen ? 'is-open' : ''}>
            ...
            <Sidebar>
                {items.length > 0 ? (
                    <CartStage className={!isCart ? 'is-checkout' : ''}>
                        <ul>
                            ...
                            <ItemCart key={p.id}>
                                ...
                                <DeleteItemButton type="button" onClick={() => removeItem(p.id)} />
                            </ItemCart>
                            ))}
                        </ul>
                        <InfosCart>
                    ...
                    <EmptyText>O carrinho está vazio, adicione algum item</EmptyText>
                )}
